...
class BreederBatch(Document):
    ...
    def get_batch_price(self):
        """Get breeder batch cost from settings"""
        batch_cost = frappe.get_value("Batch Settings", None, "breeder_batch_cost")
        if not batch_cost:
            frappe.throw("Breeder batch cost not configured in Batch Settings")
        ...
        history_doc = frappe.get_doc(
            {
                ...
                "description": f"Deduction for Breeder Batch {self.name}",
            }
        )
