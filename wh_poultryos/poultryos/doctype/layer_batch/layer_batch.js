...
frappe.ui.form.on('Layer Batch', {
    ...
    before_save: function (frm) {
        if (frm.is_new()) {
            console.log("New Layer Batch Detected in before_save");
            return new Promise((resolve, reject) => {
                checkTokenBalance(frm, resolve, reject);
            });
        } else {
            console.log("Existing Layer Batch Modified: No Count Change Needed");
        }
    },
    ...
    after_save: function (frm) {
        ...
        frappe.set_route("List", "Layer Batch");
    }
});
...
function checkTokenBalance(frm, resolve, reject) {
    ...
    frappe.call({
        ...
        callback: function (r) {
            const layer_batch_cost = r.message.layer_batch_cost;
        ...
        fields: [
            {
                ...
                options: `<p>You need <strong>${amount_needed}</strong> tokens to create a new Layer Batch.</p>`
            },
            {
                label: 'Amount Needed',
                fieldname: 'amount',
    ...
    frappe.call({
        ...
        callback: function (response) {
            let checkoutOptions = {
                ...
                returnUrl: base_url + "/app/layer-batch/new-layer-batch",
            };
