...
class LayerBatch(Document):
    def before_insert(self):
        ...
        if self.user_balance < 1:
            frappe.throw(
                f"Insufficient balance. You need 1 token but have {self.user_balance}."
            )
    ...
    def get_batch_price(self):
        """Get layer batch cost from settings"""
        batch_cost = frappe.get_value("Batch Settings", None, "layer_batch_cost")
        if not batch_cost:
            frappe.throw("Layer batch cost not configured in Batch Settings")
        ...
        history_doc = frappe.get_doc(
            {
                ...
                "description": f"Deduction for Layer Batch {self.name}",
            }
        )
