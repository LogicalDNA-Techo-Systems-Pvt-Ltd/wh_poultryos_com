frappe.listview_settings['Layer Batch'] = {
    primary_action() {
        // Get the current logged-in user's ID
        var user_id = frappe.session.user;

        // Fetch Layer Batch cost from Batch Settings
        frappe.call({
            method: 'frappe.client.get_value',
            args: {
                doctype: 'Batch Settings',
                fieldname: 'layer_batch_cost'
            },
            callback: function (response) {
                var layer_batch_cost = response.message.layer_batch_cost;
                console.log("layer_batch_cost", layer_batch_cost);
                ...
                frappe.call({
                    ...
                    callback: function (response) {
                        ...
                        if (user_balance >= layer_batch_cost) {
                            // Sufficient balance, redirect to Layer Batch creation page
                            frappe.set_route('Form', 'Layer Batch', 'new-layer-batch');
                        } else {
                            // Insufficient balance, show dialog to purchase tokens
                            let amount_needed = layer_batch_cost - user_balance;
        ...
        fields: [
            {
                ...
                fieldtype: 'HTML',
                options: `<p>You need more tokens to proceed with creating the Layer Batch.</p>`
            },
            {
                ...
                onchange: function () {
                    ...
                    dialog.set_value('message_html',
                        `<p>You need <strong>${total_amount}</strong> tokens to proceed with creating ${batch_count} Layer Batch${batch_count > 1 ? 'es' : ''}.</p>`
                    );
                }
            },
            {
